...
import ch.admin.bag.covidcode.authcodegeneration.config.security.authentication.JeapAuthenticationConverter;
import ch.admin.bag.covidcode.authcodegeneration.config.security.authentication.ServletJeapAuthorization;
import ch.admin.bag.covidcode.authcodegeneration.config.security.validation.AudienceJwtValidator;
import ch.admin.bag.covidcode.authcodegeneration.config.security.validation.ContextIssuerJwtValidator;
import ch.admin.bag.covidcode.authcodegeneration.config.security.validation.JeapJwtDecoderFactory;
...
public class OAuth2SecuredWebConfiguration {
    ...
    public static class OAuth2SecuredWebMvcConfiguration extends WebSecurityConfigurerAdapter {
        ...
        private JwtDecoder createJwtDecoder() {
            ...
            final String b2bGatewayJwkSetUri = Optional.ofNullable(commonConfiguration.getResourceServer().getB2BGateway()).
                    map(B2BGatewayConfigProperties::getJwkSetUri).orElse(null);
            return JeapJwtDecoderFactory.createJwtDecoder(authorizationJwkSetUri, b2bGatewayJwkSetUri, createTokenValidator(commonConfiguration));
        }
    }
    ...
    static ContextIssuerJwtValidator createContextIssuerJwtValidator(ResourceServerConfigProperties resourceServerConfigProperties) {
        ...
        contextIssuers.put(JeapAuthenticationContext.SYS, authorizationServer);
        contextIssuers.put(JeapAuthenticationContext.USER, authorizationServer);
        if ((resourceServerConfigProperties.getB2BGateway() != null) &&
             StringUtils.isNotBlank(resourceServerConfigProperties.getB2BGateway().getIssuer())) {
            contextIssuers.put(JeapAuthenticationContext.B2B, resourceServerConfigProperties.getB2BGateway().getIssuer());
        }
